...
public class markovClientJava {
    private int numStates; // 状态数量
    private double[] initialProbabilities; // 初始状态概率
    private double[][] transitionMatrix; // 转移矩阵
    private double[][] emissionMeans; //  emission分布均值
    private double[][] emissionVars; // emission分布方差

    /**
     * 构造函数
     * @param numStates 状态数量（通常3-5个）
     */
    public markovClientJava(int numStates) {
        ...
        for (int i = 0; i < numStates; i++) {
            initialProbabilities[i] = 1.0 / numStates + random.nextDouble() * 0.1 - 0.05;
            sumInit += initialProbabilities[i];
        }
        // 归一化初始概率
        for (int i = 0; i < numStates; i++) {
            initialProbabilities[i] /= sumInit;
        }

        // 转移矩阵：对角线元素稍大，表示更可能留在当前状态
        for (int i = 0; i < numStates; i++) {
            double sumTrans = 0;
            for (int j = 0; j < numStates; j++) {
                if (i == j) {
                    transitionMatrix[i][j] = 0.5 + random.nextDouble() * 0.2;
                } else {
                    transitionMatrix[i][j] = (0.5 - random.nextDouble() * 0.2) / (numStates - 1);
                }
                sumTrans += transitionMatrix[i][j];
            }
            // 归一化转移矩阵的行
            for (int j = 0; j < numStates; j++) {
                transitionMatrix[i][j] /= sumTrans;
            }
        }

        // 发射分布均值：使用不同的均值
        for (int i = 0; i < numStates; i++) {
            emissionMeans[i][0] = (i - numStates / 2.0) * 0.5;
        }

        // 发射分布方差：使用较小的随机值
        for (int i = 0; i < numStates; i++) {
            emissionVars[i][0] = 0.5 + random.nextDouble() * 0.5;
        }
    }

    /**
     * 训练HMM模型
     * @param observations 观测序列（收益数据）
    ...
    /**
     * 获取gamma矩阵，用于分析每个时间步的状态概率
     * @param observations 观测序列（收益数据）
     * @return gamma矩阵 [时间步][状态] 的概率
     */
    ...
    private void baumWelch(double[] observations, int maxIterations) {
        ...
        final double LEARNING_RATE = 0.03; // 学习率，用于减缓参数更新
